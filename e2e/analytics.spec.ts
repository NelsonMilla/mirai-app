...
test('the early bird funnel reports section reach and checkout intent', async ({ page }) => {
  ...
  // `offer` is first so the loop waits for its Section Viewed event rather than
  // racing the scroll away from it — under load it was registering only sometimes.
  ...
  // Section positions after the CXL redesign: offer/1, program/2, proof/3,
  // close/5. There is no pricing section and nothing sits at position 4.
  // The loop visits three, and `proof` at 3 is the deepest. `close` is not
  // counted: Checkout Opened fires before the click's scroll registers it.
